...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        self.input_embedding = nn.Embedding(num_embeddings = vocab_size, embedding_dim = dim) # TODO: Define the input embedding layer
        ...
        self.out_norm = LayerNorm(normalized_shape=dim) # TODO: Define the output layer normalization. Use the LayerNorm class defined in modeling/transformer_layers.py
        self.to_logits = nn.Linear(in_features=dim, out_features=vocab_size) # TODO: Define the output projection layer
    ...
    def forward_model(self, x: torch.LongTensor) -> torch.Tensor:
        ...
        x = x + self.positional_embedding[:L].unsqueeze(0)  # [1, L, D] broadcast to [B, L, D]
        ...
        mask = torch.tril(torch.ones(L, L, device=x.device, dtype=torch.bool), diagonal=0)
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, padding_idx: int = -100) -> torch.Tensor:
        ...
        B,L,V = logits.shape
        loss = F.cross_entropy(logits.reshape(B*L,V), target_seq.reshape(B*L), ignore_index=padding_idx)
        return loss
    ...
    def generate(
        ...
        for _ in range(self.max_seq_len - len(context)):
            ...
            logits=self.forward_model(current_tokens) # Shape [1, L, V]
            ...
            sampled_idx,_ = sample_tokens(last_logits, temperature=temp, top_k=top_k, top_p=top_p)
            ...
            current_tokens = torch.cat([current_tokens, sampled_idx], dim = 1)

            # Break if the end-of-sequence token is generated
            if eos_idx is not None and sampled_idx.item() == eos_idx:
                break
