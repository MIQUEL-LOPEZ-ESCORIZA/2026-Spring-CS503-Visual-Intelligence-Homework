...
class FourM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.enc_tok_emb = nn.Embedding(num_embeddings=self.vocab_size, embedding_dim=dim) # TODO: Define the input embedding layer using self.vocab_size
        ...
        self.enc_mod_emb = nn.Embedding(num_embeddings=self.num_modalities, embedding_dim=dim) # TODO: Define the encoder modality embedding layer using self.num_modalities
        self.dec_mod_emb = nn.Embedding(num_embeddings=self.num_modalities, embedding_dim=dim) # TODO: Define the decoder modality embedding layer using self.num_modalities
                
        # Initialize Transformer encoder and decoder trunks
        self.encoder = TransformerTrunk(dim = dim, depth = enc_depth, head_dim=head_dim, mlp_ratio=mlp_ratio, use_bias=use_bias) # TODO: Define the Transformer encoder trunk
        self.decoder = TransformerDecoderTrunk(dim = dim, depth = dec_depth, head_dim=head_dim, mlp_ratio=mlp_ratio, use_bias=use_bias) # TODO: Define the Transformer decoder trunk

        # Initialize encoder -> decoder context projection
        self.dec_context_proj = nn.Linear(in_features=dim, out_features=dim) # TODO: Define the encoder -> decoder projection. This is simply a linear layer mapping dim -> dim.

        # Initialize decoder output projection
        self.to_logits = nn.Linear(in_features=dim, out_features=self.vocab_size) # TODO: Define the output projection layer using self.vocab_size

        # Initialize norm layers
        self.enc_norm = LayerNorm(normalized_shape=dim) # TODO: Define the encoder output layer normalization. Use the LayerNorm class defined in modeling/transformer_layers.py
        self.dec_norm = LayerNorm(normalized_shape=dim) # TODO: Define the decoder output layer normalization. Use the LayerNorm class defined in modeling/transformer_layers.py
    ...
    def forward_encoder(
        ...
        enc_modembs = self.enc_mod_emb(enc_input_modalities)
    ...
    def forward_decoder(
        ...
        Args:
            ...
            dec_input_positions: LongTensor of shape (B, M) with decoder input positions,
    ...
    def generate_one_modality_roar(
        ...
        perm = torch.randperm(n_tokens_target, device=device)
        ...
        dec_input_positions_list = [chunk.unsqueeze(0) for chunk in torch.split(perm, schedule)]
        ...
        for step, k in enumerate(schedule):
            ...
            enc_pad_mask=None,
            dec_pad_mask=None, )[0]  # [k, vocab_size]
