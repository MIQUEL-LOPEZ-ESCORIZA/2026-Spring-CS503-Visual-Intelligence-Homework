...
class MaskGIT(nn.Module):
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        x = torch.where(mask.unsqueeze(-1), self.mask_token.view(1,1,-1),x) # Shape [B, L, D]
        # Shape [B num_heads L L] ?

        # TODO: Add the positional embeddings to the tokens
        x = x + self.positional_embedding[:L,:].unsqueeze(0) # Shape [B, L, D]
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        num_masked = torch.randint(1, L + 1, (B, 1), device=device) # Shpe [B,1]

        # Random per-position scores, then rank positions per sample
        scores = torch.rand(B, L, device=device)
        ranks = scores.argsort(dim=1).argsort(dim=1)  # 0..L-1 per row

        # Mask lowest `num_masked` ranks
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        B,L,V = logits.shape
        return F.cross_entropy(input = logits.reshape(B*L, V), target = target_seq.reshape(B*L), ignore_index=ignore_index)
    ...
    def forward(self, data_dict: Dict[str, Any]) -> Tuple[torch.Tensor, Dict[str, Any]]:
        ...
        print("logits have shape: ", logits.shape, " and target has shape ", target.shape)
    ...
    def get_maskgit_schedule(self, mask: torch.BoolTensor, num_steps: int = 8) -> List[int]:
        ...
        schedule = [total_tokens // num_steps] * (num_steps - 1) + [total_tokens // num_steps + total_tokens % num_steps]
